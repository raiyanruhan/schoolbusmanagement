...
function StopForm({
  ...
  routeId: string;
  initial?: Stop;
  onSubmit: (name: string, name_bn: string) => void;
  onCancel: () => void;
  loading: boolean;
  error: string | null;
  nextOrder: number;
    ...
    >
      <FormControl required>
        ...
        <TextInput
          ...
          placeholder="e.g. Railway Crossing"
          block
        />
      </FormControl>
      ...
      {!initial && (
        <Text sx={{ fontSize: 0, color: "fg.muted" }}>
          Will be added as stop #{nextOrder}
        </Text>
      )}
...
export default function RouteManagement() {
  ...
  // Drag state for reordering with enhanced UX
  ...
  const handleExcelImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.success) {
        const { routesCreated, routesExisting, stopsCreated, stopsSkipped } =
          result.data;
        showToast(
          `Imported: ${routesCreated} new route${routesCreated !== 1 ? "s" : ""}, ` +
            `${stopsCreated} new stop${stopsCreated !== 1 ? "s" : ""}` +
            (routesExisting > 0
              ? ` (${routesExisting} existing routes updated)`
              : "") +
  ...
  const handleAddStop = async (name: string, name_bn: string) => {
    ...
    const nextOrder =
      (selected.stops.length > 0
        ? Math.max(...selected.stops.map((s) => s.sequence_order))
        : 0) + 1;
  ...
  const handleEditStop = async (name: string, name_bn: string) => {
    ...
    setFormLoading(true);
    setFormError(null);
    const result = await window.api.stop.update({
      id: editStop.id,
      name: name.trim(),
      name_bn: name_bn.trim() || undefined,
    });
  ...
  // Drag-drop reorder with enhanced UX
  ...
  const handleDrop = async (targetId: string) => {
    ...
    if (dragOverPosition === "after") finalToIdx = toIdx + 1;

    // Adjust for item removing itself from before the target
    if (fromIdx < finalToIdx) finalToIdx--;

    const [moved] = stops.splice(fromIdx, 1);
    stops.splice(finalToIdx, 0, moved);

    const newIds = stops.map((s) => s.id);
    setDragging(null);
    setDragOver(null);
    setDropSuccess(dragging);

    // Clear success state after animation
    setTimeout(() => setDropSuccess(null), 500);
  ...
  return (
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
            ...
            {routes.map((route) => (
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  border: "none",
                  borderLeft: `2px solid ${selected?.id === route.id ? "var(--borderColor-accent-emphasis)" : "transparent"}`,
                  background:
      ...
      >
        {!selected ? (
          ...
          >
            ...
            <Text
              sx={{ fontWeight: "semibold", color: "fg.default", fontSize: 1 }}
            >
              Select a route
            </Text>
            <Text sx={{ fontSize: 0, color: "fg.muted" }}>
              Choose a route on the left to manage its stops
            </Text>
          </div>
        ) : (
          <>
            ...
            >
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <IconButton
                  ...
                  variant="invisible"
                  size="small"
                  onClick={() => setDeleteRoute(selected)}
                  sx={{ color: "danger.fg" }}
                />
              </div>
            </div>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: 32 }}>
                ...
                {selected.stops.length === 0 ? (
                  ...
                  <ul style={{ listStyle: "none", margin: 0, padding: 0 }}>
                    {selected.stops.map((stop, i) => {
                      ...
                      const isDragOver = dragOver === stop.id;
                      const isDropTarget =
                        dragOver !== null && dragging !== stop.id;
                        ...
                        >
                          ...
                          {isDragOver && dragOverPosition && (
                            <div
                              style={{
                                ...
                                right: "8px",
                                height: "2px",
                                background: "var(--fgColor-accent-emphasis)",
                                borderRadius: "1px",
                                top:
                                  dragOverPosition === "before" ? "0" : "auto",
                                bottom:
                                  dragOverPosition === "after" ? "0" : "auto",
                                boxShadow:
                                  "0 0 6px var(--fgColor-accent-emphasis)",
                                zIndex: 10,
                                pointerEvents: "none",
                              }}
                            />
                          ...
                          >
                            <Text
                              sx={{
                                fontSize: 0,
                                color: isDragging ? "white" : "fg.muted",
                                fontFamily: "mono",
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <Text
                              sx={{
                                ...
                                fontWeight: "medium",
                                display: "block",
                                color: stop.is_active
                                  ? isDragging
                                    ? "fg.accent-emphasis"
                                    : "fg.default"
                                  : "fg.muted",
                                textDecoration: stop.is_active
                                  ? "none"
                                  : "line-through",
                                transition: "color 0.2s ease-in-out",
                              }}
              ...
              >
                ...
                <Text sx={{ fontSize: 0, color: "fg.muted" }}>
                  Drag by the handle to reorder stops • Drop on top or bottom
                  half to position
                </Text>
              </div>
      ...
      <ConfirmDialog
        ...
        title="Delete Route"
        message={`Delete "${deleteRoute?.name}" and all its stops?`}
        confirmLabel="Delete"
        danger
      />
      ...
      >
        {selected && (
          <StopForm
            ...
            loading={formLoading}
            error={formError}
            nextOrder={(selected.stops.length || 0) + 1}
          />
        )}
      </Modal>
      <Modal
      ...
      >
        {editStop && selected && (
          <StopForm
            ...
            loading={formLoading}
            error={formError}
            nextOrder={editStop.sequence_order}
          />
        )}
      </Modal>
      <ConfirmDialog
        ...
        title="Delete Stop"
        message={`Delete stop "${deleteStop?.name}"?`}
        confirmLabel="Delete"
        danger
      />
